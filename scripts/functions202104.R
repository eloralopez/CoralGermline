setwd("~/Documents/CoralGermlineManuscript/")
library("reshape2")
library(ggplot2)
library(stringr)
library(sciplot)
...
CA60sa<-253.02
CA65sa<-704.15
files<-list.files(path="~/Documents/CoralGermlineManuscript", pattern="*56_dm_onelib_20210409_1.txt", full.names=T, recursive=FALSE)
threedfs_func<- function(files) {
  ...
  trueLoHmutantparents<- rbind(trueLoHp2) #combines trueLoHp2 and trueLoHp2.1
  ...
  correspondingsperm2<-sperm2[match(trueLoHmutantparents$chrom.pos,sperm2$chrom.pos),]#subsets just the sperm2 loci that match loci in removeuniques_mp
  trueLoH_somatic_mpandms<-rbind(trueLoHmutantparents, correspondingsperm2) #correspondingsperm1 and 2 have duplicates of each line, use unique to remove duplicates, then rbind with removeuniques_mp for the dataset that has the mutantparent and mutantsperm samples at just the loci that are true somatic LoH
  ...
  trueLoHmutantsperm<- rbind(trueLoHsperm2.1) #combines trueLoHsperm2 and trueLoHsperm2.1
  ...
  correspondingmp1<- mp1[match(trueLoHmutantsperm$chrom.pos, mp1$chrom.pos),] #subsets just the mp1 loci that match loci in removeuniques_ms
  ...
  trueLoH_uglm_mpandms<-rbind(trueLoHmutantsperm, unique(correspondingmp1)) #correspondingmp1 and 2 have duplicates of each line, use unique to remove duplicates, then rbind with removeuniques_ms for the dataset that has the mutantparent and mutantsperm samples at just the loci that are true uglm LoH
...
somaticmetadatadf<-somaticCA56df
scatterplot_func<-function(somaticmetadatadf) {
  ...
  spermaverage<-(spermprops2)#/2
  dfx<-cbind(spermdf2$chrom.pos, spermprops2,spermprops2,spermaverage)
  Inheritance = rep("A", nrow(spermdf2))
  LaxInheritance = rep("A",nrow(spermdf2))
  Inherited = rep("inherited",nrow(spermdf2))
  NotInherited = rep("notinherited",nrow(spermdf2))
  
  for (i in 1:nrow(spermdf2)){
    if (spermdf2$GoH_or_LoH[i] =="DeNovo" & spermaverage[i] >0) {
      Inheritance[i] = Inherited[i]
    } else if (spermdf2$GoH_or_LoH[i] =="DeNovo" & spermaverage[i] == 0) {
      Inheritance[i] = NotInherited[i]
    } else if (spermdf2$GoH_or_LoH[i] =="LoH" & spermdf2$mutant_allele_depth[i]/spermdf2$totaldepth.x[i] == 1 & spermdf2$mutant_allele_depth[i]/spermdf2$totaldepth.x[i] == 1) {
      Inheritance[i] = Inherited[i]
    } else if (spermdf2$GoH_or_LoH[i] =="LoH" & (spermdf2$mutant_allele_depth[i]/spermdf2$totaldepth.x[i] != 1 | spermdf2$mutant_allele_depth[i]/spermdf2$totaldepth.x[i] != 1)) {
      Inheritance[i] = NotInherited[i]
    } else (Inheritance[i] =="na")
